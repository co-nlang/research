...
**Attack plan for the hard core written (2026-06-22, `supplementary/item21_arity5/ATTACK_PLAN.md`).** The lone remaining front — the *exotic non-$\omega$ arity-5 invariant* — is laid out as an XXIII-level program: extend Paper XIX's two-prong Arf-exclusion (P1 frame-$q$ non-invariance; P2 the intrinsic-$q$ Gram/relation skeleton, "Arf$=0$ but parity splits") from arity-4/$K_5$/$H^3$ to arity-5/$K_6$/$H^4$ (6 Lagrangians, 15 rays, ray-span $W_6$). Milestones, **grounded ladder (2026-06-22, every step cross-checked against XIX/XXII + scripts — the plan had over-labelled several "open"):** **M1 DONE** (the natural arity-5 4-cochain is exact, $\langle c,[K_6]\rangle=0$ — Paper XXII Thm `thm:trunc`, $c=\delta\mathbf a$ via the 4-index *cochain-indexing* argument — purely cochain-level and **stratum-agnostic** ($\omega(v_{ij},v_{kl})$ indexed by the 4-subset $\{i,j,k,l\}\Rightarrow a$ a 3-cochain $\Rightarrow c=\delta a$, exact since $[S^4]$ is a cycle); M2's $(2n,2n,0)\!\neq\!(8,6,2)$ does *not* bear on it — verified `k6_truncation.py`); **M3 DONE-IN-SPIRIT** (frame-$q$ exclusion = XIX `Prop frameq`, arity-agnostic, lifts verbatim to 15 rays); **M2 DONE** (2026-06-22, `k6_skeleton.py`: the $K_6$ skeleton — generic stratum **$(2n,2n,0)$ full-span, nondegenerate**, structurally *unlike* XIX's degenerate $K_5$ $(8,6,2)$; structural facts verified 100%; intrinsic-$q$ existence $0\%/40\%/31\%$ at $n=4/5/6$ — handing M4 a real branch (b) at $n=4$); **M4 DONE for the Arf/P2 candidate** (2026-06-23, `m4_intrinsic.py`, `M4_intrinsic_README.md`): built the intrinsic exotic $Q_6$ (unique quadratic refinement of $\omega$ on $W_6$ vanishing on the 15 rays) + its Arf, tested vs the $H^4$ fiber ($c_m=N_{\mathrm{anti}}(\text{face }m)$, $M=N_{\mathrm{anti}}(\text{all }6)$). **Outcome (a):** the Arf candidate is *absorbed* at $n\ge5$ — at $n=5$ by being **constant** ($\mathrm{Arf}\equiv0$, the XIX $K_5$ pattern), at $n=6$ by being **reducible** ($\mathrm{Arf}$ is *non-constant* $\{0{:}39,1{:}4\}$ — validated basis-independent $0/43$, so the variation is *real* and the $K_5$ "killed by constancy" argument *fails* — but $\mathrm{Arf}$ is the Dickson polynomial in the arity-$\le4$ ray-Gram, a **structural** reducibility proof, since each $\omega(r_{ij},r_{kl})$ is $\le4$-index); at $n=4$ it **does not exist** (branch (b), existence $0\%$). The $(c)$-falsification branch did **not** occur; the genuine discovery is the **mechanism shift** constant$\to$reducible. *Scope:* M4 absorbs **one** candidate (the natural P2 exotic), not the unparametrized exotic space (O3) — it is evidence *for* item 21 and *completes the P2 lift*, but does **not** close item 21. **M5 OPEN** (a first/second fundamental theorem for $\mathrm{Sp}(2n,\mathbb F_2)$ on Lagrangian tuples — the actual item-21 theorem, paper-scale). **So the $\omega$-generated front (M1, M3), the structural scaffold (M2), *and* the natural P2 exotic candidate (M4) are settled; the genuinely-open content is M5 alone (the uniform no-exotic theorem).**
