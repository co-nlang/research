...
**Attack plan for the hard core written (2026-06-22, `supplementary/item21_arity5/ATTACK_PLAN.md`).** The lone remaining front — the *exotic non-$\omega$ arity-5 invariant* — is laid out as an XXIII-level program: extend Paper XIX's two-prong Arf-exclusion (P1 frame-$q$ non-invariance; P2 the intrinsic-$q$ Gram/relation skeleton, "Arf$=0$ but parity splits") from arity-4/$K_5$/$H^3$ to arity-5/$K_6$/$H^4$ (6 Lagrangians, 15 rays, ray-span $W_6$). Milestones, **grounded ladder (2026-06-22, every step cross-checked against XIX/XXII + scripts — the plan had over-labelled several "open"):** **M1 DONE** (the natural arity-5 4-cochain is exact, $\langle c,[K_6]\rangle=0$ — Paper XXII Thm `thm:trunc`, $c=\delta\mathbf a$ via the 4-index partition argument, verified `k6_truncation.py`); **M3 DONE-IN-SPIRIT** (frame-$q$ exclusion = XIX `Prop frameq`, arity-agnostic, lifts verbatim to 15 rays); **M2 OPEN** ($K_6$ Gram/relation skeleton — confirmed new, no $K_6$ skeleton in XIX/XXII); **M4 OPEN** (intrinsic exotic at $K_6$ — the crux, the non-4-index-decomposable remainder; no $K_6$ intrinsic-$q$/Arf anywhere); **M5 OPEN** (a first/second fundamental theorem for $\mathrm{Sp}(2n,\mathbb F_2)$ on Lagrangian tuples — the actual item-21 theorem, paper-scale). **So the entire $\omega$-generated front (M1, M3) is already settled; the genuinely-open content is M2 (scaffold) + M4 (crux) + M5 (theorem).** Obstacles unchanged (O1–O4 of "Why it is genuinely hard"). M1–M4 are evidence/scaffold; only M5 *closes* item 21.
