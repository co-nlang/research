...
**A positive line of attack — the Steenrod-module route (2026-06-21).** 23 turns item 21 from a two-front problem into a *one-front* problem and points at the weapon. The conditional is clean: **if** the family-A resonance tower is generated by $\omega$ *as a module over the Steenrod algebra* in the relevant range, **then** $\mathrm{Sq}^1\omega$ being the unique indecomposable lift and $\mathrm{Sq}^1\mathrm{Sq}^1\omega=0$ cap it at $H^3$ — no arity-5 escape. The Steenrod-generated front is thereby *already closed*; the entire residual is the single hypothesis **"the tower is $\omega$-generated over $\mathcal A$,"** i.e. the exotic invariants get *absorbed* (do not lift to irreducible arity-5 / $H^4$ obstructions). The concrete test object is **Paper XIX's $q_4$**: it is the known arity-4 exotic (Arf/Dickson-type), so the program is to show $q_4$ — and any sibling exotic — is, in the Steenrod-module sense, *not* an independent generator climbing to $H^4$ but is subsumed by the $\omega$-orbit (or else dies by $\mathrm c=\delta\mathrm a$ exactness, Paper XXII). This is the **first positive route 21 has had**: not "enumerate / rule out a family" (which only nudges confidence) but "prove a module-generation statement" — a structural target with a definite shape, even though controlling the unclassified exotic part remains the hard step. (Caveat, kept honest: this is a *route*, not a result; the absorption of $q_4$ is itself unproven, and "$\omega$-generated over $\mathcal A$" is precisely the gap, not a given.)
