...
**Attack plan for the hard core written (2026-06-22, `supplementary/item21_arity5/ATTACK_PLAN.md`).** The lone remaining front — the *exotic non-$\omega$ arity-5 invariant* — is laid out as an XXIII-level program: extend Paper XIX's two-prong Arf-exclusion (P1 frame-$q$ non-invariance; P2 the intrinsic-$q$ Gram/relation skeleton, "Arf$=0$ but parity splits") from arity-4/$K_5$/$H^3$ to arity-5/$K_6$/$H^4$ (6 Lagrangians, 15 rays, ray-span $W_6$). Milestones M1 (the natural arity-5 4-cochain is exact, $\langle c,[K_6]\rangle=0$ — **already a theorem: Paper XXII Thm `thm:trunc`, $c=\delta\mathbf a$**, via the 4-index partition argument; verified `k6_truncation.py`. *Not open* — it closes the $\omega$-generated/4-index-decomposable route and pinpoints M4's opening) → M2 ($K_6$ skeleton lemma) → M3 (frame-$q$, cheap) → M4 (intrinsic exotic at $K_6$, the genuine computational crux — the non-4-index-decomposable remainder) → **M5 (a first/second fundamental theorem for $\mathrm{Sp}(2n,\mathbb F_2)$ on Lagrangian tuples — the actual item-21 theorem, paper-scale).** Obstacles unchanged (O1–O4 of "Why it is genuinely hard"). M1–M4 are evidence/scaffold; only M5 *closes* item 21.
