...
**Update — the bridge CLOSES, modulo one cited theorem (2026-06-21, `supplementary/item23_search/{closing_note.md,sq1_bar.py}`).** The "intrinsic identification" is now resolved into a three-link chain $(n_a)_m = q(S_m)\oplus\bigoplus q \;=\; (c\cup_1 c)|_m \;=\; \mathrm{Sq}^1\omega|_m$, with an honest ledger: **(A)** $(n_a)_m=$ polarization defect of $q$ — *new, proven all $n$* (123k/123k); **(C)** that defect $=\mathrm{Sq}^1\omega$ — *all $n$, by Steenrod's definition + Cartan* (not an empirical small-$n$ fact): $c=\sum_i a_i\!\cup b_i$ ($c(g_1,g_2)=X_{g_1}\!\cdot\!Z_{g_2}$) is a cup of 1-cocycles, hence a 2-cocycle with $[c]=\omega$, $q(v)=c(v,v)$ its diagonal; $\mathrm{Sq}^1[c]:=[c\cup_1 c]$ is the *definition*; $\mathrm{Sq}^1\omega=\sum_i a_ib_i(a_i+b_i)$ by Cartan. `sq1_bar.py` is the *convention unit-test* of the coded $\cup_1$ formula (an $n$-independent expression): it confirms the formula yields the nonzero $\mathrm{Sq}^1\omega$ — $[c\cup_1 c]=[\mathrm{Sq}^1\omega]$ ($n=1$: 11/16; $n=2$: 234/256; $\delta r$ consistent, and the $\delta r$ witness need not generalize) — with $c$ a 2-cocycle confirmed at $n=1,2,3,4$ (into the $K_5$-rigidity regime). The $n$-dependence of $[n_a]=0\iff n=4$ lives *not here* but in (A) (in-regime $n=4,5,6$) and the master theorem; **(B)** the join is the **Kudo transgression** $\tau(t^2)=d_3(t^2)=\mathrm{Sq}^1\omega$ of the Heisenberg LHS spectral sequence ($q$ the chain trivialization, $\delta q=\omega$) — *classical (Quillen/Kudo), cited not re-derived*, the Paper XXII outlook backbone. **Net:** the bridge no longer needs a from-scratch lax/$A_\infty$ map — the configuration-side obstruction has an explicit elementary cochain (A), provably the standard $\mathrm{Sq}^1\omega$ (C), meeting at the transgression (B). *Not claimed:* re-deriving Quillen/Kudo. The $\phi^*\omega\equiv0$ flag stands (the bridge is genuinely *secondary*, zero-indeterminacy $H^1(S^3)=0$; $q$ is its nullhomotopy datum — why a primary symplectic formula was provably circular). **Item 23's open half is closed to a cited classical transgression; the genuinely-open frontier reduces to item 21.**
