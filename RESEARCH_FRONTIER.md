...
**Update — the bridge CLOSES, modulo two cited theorems + one expansion-check (2026-06-21, `supplementary/item23_search/{closing_note.md,sq1_bar.py}`).** The "intrinsic identification" is now resolved into a three-link chain $(n_a)_m = q(S_m)\oplus\bigoplus q \;=\; (c\cup_1 c)|_m \;=\; \mathrm{Sq}^1\omega|_m$, with an honest ledger: **(A)** $(n_a)_m=$ polarization defect of $q$ — *new, proven all $n$* (123k/123k); **(C)** that defect $=\mathrm{Sq}^1\omega$ — *all $n$, by Steenrod's definition + Cartan* (not an empirical small-$n$ fact): $c=\sum_i a_i\!\cup b_i$ ($c(g_1,g_2)=X_{g_1}\!\cdot\!Z_{g_2}$) is a cup of 1-cocycles, hence a 2-cocycle with $[c]=\omega$, $q(v)=c(v,v)$ its diagonal; $\mathrm{Sq}^1[c]:=[c\cup_1 c]$ is the *definition*; $\mathrm{Sq}^1\omega=\sum_i a_ib_i(a_i+b_i)$ by Cartan. `sq1_bar.py` is the *convention unit-test* of the coded $\cup_1$ formula (an $n$-independent expression): it confirms the formula yields the nonzero $\mathrm{Sq}^1\omega$ — $[c\cup_1 c]=[\mathrm{Sq}^1\omega]$ ($n=1$: 11/16; $n=2$: 234/256; $\delta r$ consistent, and the $\delta r$ witness need not generalize) — with $c$ a 2-cocycle confirmed at $n=1,2,3,4$ (into the $K_5$-rigidity regime). The $n$-dependence of $[n_a]=0\iff n=4$ lives *not here* but in (A) (in-regime $n=4,5,6$) and the master theorem; **(B)** the join is a **secondary operation** — it *must* be: $H^2(S^3)=0\Rightarrow f^*\omega=0\Rightarrow f^*\mathrm{Sq}^1\omega=0$ as a primary class, so $N_{\mathrm{anti}}\neq0$ can only be secondary (this is *why* $\phi^*\omega\equiv0$). $q$ is the nullhomotopy, automatically ($q$'s quadratic-refinement identity *is* $\delta q=\omega$). It needs **two** distinct cited inputs, not one: **(B1)** *Steenrod–Epstein's functional-operation formula* (explicit cochain shape $c\cup_1 c$ + corrections in $q$ — what the operation looks like) and **(B2)** *Kudo's transgression* $\tau(t^2)=\mathrm{Sq}^1\omega$ (what it equals; Quillen extra-special $2$-groups, the Paper XXII backbone). **Net:** the bridge no longer needs a from-scratch lax/$A_\infty$ map — explicit elementary cochain (A), provably standard $\mathrm{Sq}^1\omega$ (C), meeting as a secondary operation (B1+B2). *Not claimed:* re-deriving Steenrod–Epstein or Quillen/Kudo, **and** the single remaining minimal step — expanding the Steenrod–Epstein formula on $(c,q)$ to match (A) term-by-term ("expand-known-formula-and-check-algebra," same difficulty class as the closed links, not yet done). **Item 23's open half is closed to two cited classical results plus one expansion-check; the genuinely-open frontier reduces to item 21.**
