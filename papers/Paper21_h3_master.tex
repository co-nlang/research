...
The finite-geometric substrate---the $n$-qubit Pauli group as the symplectic polar
space $W(2n-1,\mathbb{F}_2)$---is due to Saniga and Planat~\cite{SanigaPlanat}, and
the quantitative study of contextuality on these spaces (the \emph{contextuality
degree}, a sibling of $N_{\mathrm{anti}}$) to de Boutray \emph{et al.}~\cite{deBoutray}
and Muller \emph{et al.}~\cite{Muller}; the latter compute degrees configuration-by-
configuration through $n=6$. The master theorem below is of a different type: it is a
\emph{rigidity} statement ($N_{\mathrm{anti}}=10$ universally $\iff n=4$) and a
structural \emph{$n$-dependence} (the even/odd carrier dichotomy), expressed through
the $H^3$ class $n_a=\delta\mu$---group cohomology of $\mathrm{Sp}(2n,\mathbb{F}_2)$
via the Maslov/Wall machinery, distinct from the Veldkamp space of the finite-geometry
program and from measurement-level sheaf cohomology.
