...
This paper works within the finite geometry of the $n$-qubit Pauli group as the
symplectic polar space $W(2n-1,\mathbb{F}_2)$, due to Saniga and
Planat~\cite{SanigaPlanat}. The invariant $N_{\mathrm{anti}}$ is a sibling of the
\emph{contextuality degree} computed for these spaces by de Boutray, Holweck,
Giorgetti, Masson and Saniga~\cite{deBoutray} and Muller \emph{et
al.}~\cite{Muller}, whose case-by-case computations include the four-qubit
quadrics. Our contribution is complementary and structural: rather than bounding a
degree configuration-by-configuration, we prove that $N_{\mathrm{anti}}$ is
\emph{universal} ($=10$) across \emph{all} proper $K_5$s at $n=4$ --- a rigidity
statement about the $n=4$ stratum, not a single computation.
