...
The identification of the $n$-qubit Pauli group (modulo phase) with the symplectic
polar space $W(2n-1,\mathbb{F}_2)$, in which operator commutation corresponds to
collinearity, is due to Saniga and Planat~\cite{SanigaPlanat}; the finite geometry
of Mermin pentagrams in $W(5,\mathbb{F}_2)$, their link to three- and four-qubit
observables and to the black-hole/qubit correspondence, and the count of $12{,}096$
pentagrams used above, were developed by L\'evay, Planat and
Saniga~\cite{LevayPlanatSaniga}. The present paper works entirely within this
substrate; its contribution is the \emph{algebraic} (rather than enumerative)
origin of the obstruction.

The cross-context invariant $N_{\mathrm{anti}}$ is a sibling of the
\emph{contextuality degree} of a configuration --- the minimal number of contexts
whose $\pm1$ signs cannot be made jointly consistent --- computed for multi-qubit
symplectic polar spaces by de Boutray, Holweck, Giorgetti, Masson and
Saniga~\cite{deBoutray} and sharpened by Muller \emph{et al.}~\cite{Muller}. Both
are scalar measures of contextuality on the same geometry: $N_{\mathrm{anti}}$
counts cross-context anticommuting ray pairs (this paper shows all $15$ anticommute,
forcing the KS obstruction), whereas the contextuality degree counts unsignable
contexts. The $n$-dependence of $N_{\mathrm{anti}}$ developed in the sequels
(Papers~XVIII--XXII) is the structural counterpart of the case-by-case
contextuality-degree computations of~\cite{deBoutray,Muller}.

The cohomological reading pursued in those sequels (the $H^3$ class
$n_a=\delta\mu$) is \emph{group} cohomology of $\mathrm{Sp}(2n,\mathbb{F}_2)$ via
the Maslov/Wall machinery; it is distinct both from the \emph{Veldkamp space} of
geometric hyperplanes (the dual construction of the finite-geometry program) and
from the measurement-level \v{C}ech sheaf cohomology of contextuality.
...
\item \textbf{KS no-coloring and the Petersen $=$ Kneser $K(5,2)$ structure.}
      The 15 cross-context pairs are the edges of the Petersen graph, which is the
      Kneser graph $K(5,2)$.  Note that Petersen is $3$-vertex-colorable
      ($\chi=3$, the Kneser value $n-2k+2$); what is special is that it is the
      smallest \emph{snark} --- bridgeless cubic and not $3$-edge-colorable,
      $\chi'=4$.  The KS theorem asserts that the 10-ray system has no binary
      valuation consistent with the measurement constraints.  Is there a direct
      combinatorial proof linking the KS no-valuation to a Petersen property ---
      its snark (edge-coloring) character, or the Borsuk--Ulam / Kneser lower
      bound, under which the chromatic number of $K(n,k)$ is a $\mathbb{Z}/2$
...
\bibitem[SanigaPlanat]{SanigaPlanat}
M. Saniga and M. Planat,
``Multiple Qubits as Symplectic Polar Spaces of Order Two,''
\textit{Adv. Stud. Theor. Phys.} \textbf{1} (2007), 1--4;
arXiv:quant-ph/0612179.
...
\textit{J. High Energy Phys.} \textbf{09} (2013), 037;
arXiv:1305.5689.

\bibitem[deBoutray]{deBoutray}
H. de Boutray, F. Holweck, A. Giorgetti, P.-A. Masson, and M. Saniga,
``Contextuality degree of quadrics in multi-qubit symplectic polar spaces,''
\textit{J. Phys. A: Math. Theor.} \textbf{55} (2022), 475301;
arXiv:2105.13798.

\bibitem[Muller]{Muller}
A. Muller, M. Saniga, A. Giorgetti, H. de Boutray, and F. Holweck,
``New and improved bounds on the contextuality degree of multi-qubit configurations,''
arXiv:2305.10225 (2023).
