...
\begin{remark}[KS Contradiction: Explicit Parity Count]
\label{rem:ks-parity}
The Kochen--Specker contradiction requires an \emph{odd} number of
contexts with product $-\mathbf{I}$. For the explicit pentagram in
Table~\ref{tab:pentagram}, direct computation of the five context
products via Pauli matrix multiplication shows that \textbf{all five
contexts evaluate to $-\mathbf{I}$} (verified in the supplementary
code~\cite{PaperXsuppl}).

Summing the five $\mathbb{Z}/2$ eigenvalue equations:
the left-hand side is $0$ (each of the 10 operators appears in exactly
two contexts, so each eigenvalue is counted twice);
the right-hand side is $1+1+1+1+1 = 5 \equiv 1 \pmod{2}$.
Hence $0 = 1 \pmod{2}$: no consistent $\pm 1$ assignment exists,
confirming $[f_3] \neq 0$.
\end{remark}
