...
The sorted tuple $\kappa(\mathbf{P})$ is a $G_2(2)$-invariant of the pentagram,
as directly verified by the full 12{,}096-pentagram enumeration
\cite{PaperXIVsuppl}.
\textbf{Note (correction):}
An earlier version of this remark justified this by claiming that $G_2(2)$
preserves $V$; Paper~XV (Theorem~3.1) subsequently established that $G_2(2)$
acts on the 135 Lagrangians in two orbits of sizes 72 and 63, with
$V$ in the 72-element orbit, so $G_2(2)$ does \emph{not} fix $V$ setwise.
The computational verification stands, but the algebraic explanation of why the
sorted $k$-profile is nevertheless invariant remains an open problem.
\end{remark}
