...
phase). Its action on the Hilbert space factors as a product of $N=5$
single-qubit Cliffords. The associator
$\Delta = (\mathbf{T}_{12}\mathbf{T}_{23})\mathbf{T}_{34} -
\mathbf{T}_{12}(\mathbf{T}_{23}\mathbf{T}_{34})$ measures the failure of
the three transitions to form a consistent 2-cocycle on the tetrahedron.
By the LHS spectral sequence, this 3-cochain represents the generator
of $H^3(\bar{\mathcal{P}}_5, U(1)) \cong \mathbb{Z}/2$.
\end{proof}
...
\begin{corollary}[Dynamical necessity of associativity]
\label{cor:ls-H3-dynamical}
In any universe supporting stable classical observation (i.e., permitting
well-defined L-S contraction analysis), the division algebra of quantum
amplitudes must be associative. Together with the $d_2$ argument of
Paper VI~\cite{PaperVI} (which selects $\mathbb{C}$ from the associative
division algebras), the unique possibility is $\mathbb{C}$.
\end{corollary}
