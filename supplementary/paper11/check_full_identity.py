#!/usr/bin/env python3
"""
Verify the full identity: ∑h ≡ ∑Q + ω_ns + c_sharing (mod 2)
and check ω_ns parity directly.
"""
...
def main():
                    ...
                    for p in adj[m]:
                        ...
                        for (a1,b1), r1 in rays.items():
                            for (a2,b2), r2 in rays.items():
                                ...
                                if set([a1,b1]) & set([a2,b2]):  # sharing
                                    w = omega_int(r1, r2)
                                    sum_h += w // 2
                                    omega_sharing += w
                                    c_sum += correction(r1, r2)

                        # Compute ω_ns (non-sharing pairs)
                        omega_ns = 0
                        for (a1,b1), r1 in rays.items():
                            for (a2,b2), r2 in rays.items():
                                if (a1,b1) >= (a2,b2): continue
                                if not (set([a1,b1]) & set([a2,b2])):  # non-sharing
                                    omega_ns += omega_int(r1, r2)
                        ...
                        if omega_sharing % 2 == 1:
                            omega_sharing_odd += 1
                        else:
                            omega_sharing_even += 1

                        # Check identity: ∑h ≡ ∑Q + ω_ns + c_sharing (mod 2)
                        sum_q = sum(Q_form(r) for r in rays_list)
                        rhs = sum_q + omega_ns + c_sum
    ...
    print(f"{'=' * 70}")
    print(f"Identity ∑h ≡ ∑Q + ω_ns + c_sharing:")
    print(f"  Holds: {identity_holds} ({100*identity_holds/n_pent:.1f}%)")
    print(f"  Fails: {identity_fails} ({100*identity_fails/n_pent:.1f}%)")
    print(f"\nω_ns parity:")
    print(f"  Odd:  {omega_ns_odd} ({100*omega_ns_odd/n_pent:.1f}%)")
    print(f"  Even: {omega_ns_even} ({100*omega_ns_even/n_pent:.1f}%)")
    print(f"\nω_sharing parity:")
    print(f"  Odd:  {omega_sharing_odd} ({100*omega_sharing_odd/n_pent:.1f}%)")
    print(f"  Even: {omega_sharing_even} ({100*omega_sharing_even/n_pent:.1f}%)")
