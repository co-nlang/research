#!/usr/bin/env python3
"""
Verify the two conditions needed for the parity theorem proof:
1. ∑Q(r) ≡ 0 (mod 2) for all 12,096 pentagrams
2. c_sharing ≡ 0 (mod 2) for all 12,096 pentagrams

Uses infrastructure from quadratic_refinement_v2.py
"""
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("Paper XI: Verify Proof Conditions")
    print("=" * 70)

    print("\n[1/2] Enumerating Lagrangians and contexts...")
    ...
    print(f"\n[2/2] Checking conditions on pentagrams...")
    ...
    for i in range(n_ctx):
            ...
            for k in adj[j]:
                ...
                for m in adj[k]:
                    ...
                    for p in adj[m]:
                        ...
                        # Condition 2: c_sharing mod 2
                        # Sharing pairs = pairs of rays that share a K5 vertex
                        c_sum = 0
        ...
        if n_pent > 0 and n_pent % 2000 == 0:
            print(f"  ... {n_pent} pentagrams checked")

    print(f"\n{'=' * 70}")
    print(f"Results ({n_pent} pentagrams):")
    print(f"{'=' * 70}")
    print(f"∑Q(r) even: {sum_q_even} ({100*sum_q_even/n_pent:.1f}%)")
    print(f"∑Q(r) odd:  {sum_q_odd} ({100*sum_q_odd/n_pent:.1f}%)")
    print(f"c_sharing even: {c_sharing_even} ({100*c_sharing_even/n_pent:.1f}%)")
    print(f"c_sharing odd:  {c_sharing_odd} ({100*c_sharing_odd/n_pent:.1f}%)")

    if sum_q_odd == 0 and c_sharing_odd == 0:
        print("\n✓ Both conditions hold for ALL pentagrams!")
        print("  ∑Q(r) ≡ 0 (mod 2) and c_sharing ≡ 0 (mod 2)")
        print("\nProof complete:")
        print("  ∑h ≡ ∑Q + ω_ns + c_sharing ≡ 0 + 1 + 0 = 1 (mod 2)")
    else:
        print("\n✗ Conditions do NOT hold universally")
