...
check the algebra." We *ran* it. It fails — for a sharp structural reason — but, **scoping
carefully** (the same caution as the earlier "no summary statistic determines $a$" overstatement):
two of the findings are robust facts and one is a *bounded-family* negative, i.e. evidence, not a
nonexistence proof.

1. **The self-representation map is not simplicial** — in all $175{,}000$ sampled configs the
   triangle defect $w_{ijk}=r_{ij}+r_{jk}+r_{ik}\neq0$ ($0/175{,}000$ ever zero); the rays do not
   compose. *(Robust empirical; structurally expected, not proved for all configs/$n$.)*
2. **Hence $f^\#c$ is not a cocycle on the nerve** ($\delta(f^\#c)=0$ only $\sim50\%$ — chance).
   This is the load-bearing point: the functional-operation construction *requires* $f^*\omega=0$ at
   cochain level, i.e. $f^\#c=\delta b$ *closed* — it is not, so that construction **has no input
   $b$**. (The primary $f^\#(c\cup_1 c)$ is likewise not closed; its cycle-sum is chance-distributed,
   not $N_{\mathrm{anti}}$.) *So the specific functional-$\mathrm{Sq}^1$ route is inapplicable here.*
3. **The correction is not in the bounded defect-term family we tested.** Searching
   $\mathrm{CORR}_m=(n_a)_m\oplus\mathrm{PRIMARY}_m$ over the $\mathbb F_2$-span of
   $\{1,\,q(r_e),\,q(w_f),\,c(w_f,w_g),\,\omega(w_f,w_g),\,\omega(r_e,w_f)\}$ is **inconsistent**
   ($\sim50\%$ contradictory rows at $n=4,5$, rank $12$–$13$ of $29$–$53$, cross terms don't help).
   *This is a definite negative about **this family** — not a proof that no finite local formula
   exists* (the search space of possible formulas is unparametrized; cf. the item-21 caution).

**What this means (honest, scoped).** Findings 1–2 show the *functional-$\mathrm{Sq}^1$
construction* does not apply on the nerve (no nullhomotopy of $f^\#c$); finding 3 shows the natural
bounded family of local defect-corrections does not contain the answer. Together these are **strong
evidence** — and consistent with the secondary/circular structure (`phi_omega_zero.py`) — that no
finite local cochain formula on the nerve realizes (B), so the content lives in the higher-coherence
$d_3$. **This is evidence, not a nonexistence proof.** What is *certain*: (A) [proven] and (C) [all
$n$] stand, and (B) stands on Kudo's transgression as a *cited theorem* — independently of whether a
finite shortcut is ever found. So the earlier "+ one expansion-check, same difficulty class as the
closed links" was too optimistic: the obvious expansion-check does not work, and we have no finite
reduction of (B) in hand.
...
| (B1) | (A) $=$ the $q$-secondary operation's cochain | **no finite reduction in hand** — $f^\#c$ is not closed (self-rep map not simplicial, all samples), so the functional-$\mathrm{Sq}^1$ route is inapplicable; and the bounded defect-term family tested doesn't contain the correction (`se_search.py`, inconsistent). *Strong evidence (not proof)* the operation is the higher-coherence $d_3$. |
...
transgression (B2). What the expansion attempt adds is a *characterisation of the remainder*: the
obvious finite shortcut (the functional-$\mathrm{Sq}^1$ route) is inapplicable ($f^\#c$ not closed),
and the natural bounded defect family doesn't carry it — strong evidence the join is the
higher-coherence $d_3$. So the final status is **closed modulo the cited Kudo/Quillen
transgression**; we have *no finite reduction of (B) in hand*, and good evidence (not proof) that
none of the obvious local forms works.
...
recompute $d_3$ by hand, only reproducing the cited theorem; (ii) we do **not** claim to have proved
no finite local formula for (B) exists — only that the obvious functional-$\mathrm{Sq}^1$ route is
inapplicable ($f^\#c$ not closed) and that the bounded defect family we searched (`se_search.py`)
does not contain it. The honest flag from `phi_omega_zero.py` stands and is now *explained and
sharpened*:
$\phi^*\omega\equiv0$ and $f^*\mathrm{Sq}^1\omega=0$ as a primary class, so the bridge is genuinely a
*secondary* operation; $q$ is its nullhomotopy datum at the *symmetrized* level, which is why a
primary (symplectic-pairing) formula was circular — and the evidence above suggests the obvious
local secondary (defect) forms are blocked too. The content appears to live in the higher-coherence
$d_3$ transgression; that is the working picture, with Kudo supplying the rigorous value.
...
- `se_expand.py` — the expansion attempt: self-rep map not simplicial ($w_{ijk}\neq0$ in all $175$k samples), $f^\#c$ not closed (so the functional-$\mathrm{Sq}^1$ route is inapplicable), primary cup-1 $\neq N_{\mathrm{anti}}$ (chance).
- `se_search.py` — bounded-family negative: the secondary correction is **not** in the $\mathbb F_2$-span of the tested defect terms (inconsistent at $n=4,5$) — evidence, not proof, that no finite local formula exists.
