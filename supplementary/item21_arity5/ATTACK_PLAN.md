...
= extend XIX's arity-4 Arf-exclusion to arity 5.
...
1. **`q₅` exactness, not just saturation (the Paper XXII `c=δa` route).** Saturation says `q₅` is
   *constant*; the stronger statement item 21 wants is that the arity-5 *cochain* is `δ` of an arity-4
   cochain (exact ⟹ no `H⁴` class). Compute, on `K₆` configs, whether the natural arity-5 4-cochain
   `c₅` satisfies `c₅ = δ(arity-4 cochain)` — i.e. `⟨c₅,[K₆]⟩ = 0` for all `K₆`. *(This is the direct
   `H⁴`-analog of `⟨n_a,[K₅]⟩=N_anti`; finite per-config check. Milestone M1.)*
...
4. **Intrinsic exotic exclusion at arity 5 (P2).** The hard one: does an intrinsic arity-5 exotic
   (a Dickson/Arf-type invariant of `W₆` vanishing on the 15 rays) exist, and if so is it
   constant/exact on the `H⁴` fiber? Replicate the `Arf=0-but-parity-splits` analysis at `K₆`.
   *(Milestone M4 — the crux.)*
...
- **M4 (intrinsic exotic P2)**: the crux of the *computational* evidence; replicate `Arf=0/parity-
  split` at `K₆`. Moderate.
